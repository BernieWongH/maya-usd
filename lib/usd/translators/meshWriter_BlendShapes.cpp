...
{
    MObjectArray targetMeshes;          // The target shape(s) to hit. (i.e. multiple shapes would be using Maya's "in-betweens" feature.)
    MIntArray inputTargetGroupIndices;  // The input group indices at which each target is connected under.
    MIntArray inputTargetIndices;       // The input indices at which each target is connected under.
    MIntArray targetItemIndices;        // The Maya blendshape weight indices for the resulting deformed mesh shape.
...
{
    MObject deformedMesh;          // The resulting deformed mesh shape (i.e. deformation + weight + envelope) Maya calls this a base object.
    ...
    unsigned int outputGeomIndex;  // The logical index at which the deformed mesh is ultimately connected downstream from the blendshape deformer.
};
...
VtIntArray getUnionOfVtIntArrays(const VtIntArray* const arrays, const size_t count)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        for (size_t j = 0; j < array.size(); ++j) { ++(visitedMap[array[j]]); }
    }
    ...
    std::vector<VtVec3fArray>&       unionNormalsArrays)
{
    ...
    for (size_t i = 0; i < numArrays; ++i) {
        ...
        for (size_t j = 0; j < array.size(); ++j) { ++(visitedIndicesMap[array[j]]); }
    }
